...
train_path = os.path.join(DATA_PATH, "icwb2-data/training/pku_training.utf8")
...
class HMMChineseSeg:
    ...
    def __init__(self):
        self.emit_map = list(map(lambda x: {}, self._STATUS))
        self.trans_matrix = list(map(lambda x: list(map(lambda y: 0, self._STATUS)), self._STATUS))
        self.init_status = list(map(lambda x: 0, self._STATUS))
        print(self.emit_map, self.trans_matrix, self.init_status)
    ...
    def add_train_line(self, line):
        ...
        pairs = list(zip("".join(items), status))
    ...
    def segment(self, line, seg_char=' '):
        if not line:
            return ""
        path = [list(map(lambda x: -1, self._STATUS))]
        last_max = []
        ...
        for i in range(1, len(line)):
            this_path = list(map(lambda x: -1, self._STATUS))
            this_max = list(map(lambda x: self._DEFAULT_LOG_PROB, self._STATUS))
